# BULWORK MODE — browser extension (Phase 2 MVP)

The extension is the **enforcement surface**; the local **bulwork service** is the brain. The extension
holds no secrets and runs no AI — it asks the service (`127.0.0.1:7373`) for every decision.

```
popup / content scripts  ──►  background worker  ──►  bulwork service (:7373)  ──►  ledger-cli + Claude
...
1. **Start the service** (needs `OPENROUTER_API_KEY` — or `ANTHROPIC_API_KEY` for the fallback
   provider — in `bulwork/.env` for real Tier-2 adjudication; without any key Tier-2 is a
...
2. **Load the extension** (Chrome / Edge / Brave):
   ...
   - **Load unpacked** → select `bulwork/extension/`
3. Click the BULWORK MODE toolbar icon → pick a **project** (or free-type a task, or "most recently
...
- **Settings page** (`chrome://extensions` → BULWORK MODE → Details → Extension options): service
  status, **adjudicator model picker**, "Ask about BULWORK" help panel, session feedback toggles,
...
1. **Service up:** `cd bulwork && npm run serve` → `bulwork service on http://127.0.0.1:7373 (... key: set)`.
   Leave it running.
2. **Load:** `chrome://extensions` → Developer mode ON → **Load unpacked** → pick `bulwork/extension/`.
...
4. **Tier-1 (instant block):** visit `https://reddit.com` → you should be redirected to the BULWORK
...
6. **Tier-2 (AI adjudication, soft overlay):** visit a site that's clearly off-topic *for your focus*
   ...
   **"Back to BULWORK MODE" modal** appears with **Just 1 more minute** / **Back to work →**. "1 more
...
9. **Settings:** `chrome://extensions` → BULWORK MODE → **Details** → **Extension options** → edit the
...
- Service-worker logs/errors: `chrome://extensions` → BULWORK MODE → click **service worker** → Console.
...
- **Firestore / Ledger-native store** is stubbed; sessions log locally to `bulwork/.data/sessions.jsonl`
  and plans/templates live in `bulwork/.data/*.json` — Epic D is the swap.
- **Memory-Hub grounding** (sharper adjudication) is inert unless `BULWORK_MEM_BIN` is set — see README.
