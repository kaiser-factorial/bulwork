...
## What is BULWORK MODE?

BULWORK MODE is a focus-enforcement tool: you declare one focus task, start a timed work session, and
BULWORK decides — per page you visit — whether that page is consistent with your focus. Clear
...
false block costs more than a false allow, so when in doubt BULWORK asks or allows — and if the service
is down, everything fails open (nothing is ever hard-blocked by an outage).

## Starting and stopping a focus session

Click the BULWORK MODE toolbar icon to open the popup. Pick a focus three ways: choose a **Ledger
...
re-checked: an off-task tab you left open gets the soft "Back to BULWORK MODE" overlay rather than a
