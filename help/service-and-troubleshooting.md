...
From the `bulwork/` directory: `npm install` once, then `npm run serve`. The service listens on
...
the `bulwork/extension/` folder. Pin the toolbar icon. After pulling code changes, press the reload
(↻) icon on the extension card and refresh any open tabs you're testing on.

## "Service not reachable" in the popup or options

Start (or restart) the service: `cd bulwork && npm run serve`. The extension fails open while the
...
## Where does BULWORK store its data?

Everything lives locally under `bulwork/.data/`: `tiers.json` (your tier lists), `settings.json`
...
## What does BULWORK send to the model?
...
Help questions from the "Ask about BULWORK" panel go to the same configured provider along with
excerpts of BULWORK's own documentation.

## Debugging the extension

Service-worker console: chrome://extensions → BULWORK MODE → **service worker**. Content-script logs:
