...
## How does BULWORK decide if a page is on-topic?
...
model less to go on. Before any model call, BULWORK checks its learned decisions and the tier lists —
a remembered answer short-circuits instantly with no API call.

## The clarify card ("Is this on-topic?")

When your focus is too vague to judge a page against (say the focus is just "assignment"), BULWORK
...
this focus (useful to pre-teach before BULWORK ever asks). Both persist until you clear learned
decisions.

## Viewing and clearing what BULWORK has learned
...
**rabbit-hole domains** list (options → Focus tuning; default youtube.com), BULWORK counts foreground
